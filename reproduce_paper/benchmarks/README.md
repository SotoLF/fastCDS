...
a user actually waits for, and a fair one (it charges fastCDS for its own load).

The only thing to keep in mind is to **state N**, because the ratio grows with N:
fastCDS's ~1.2 s index load is amortized over more queries as N rises, while
...
| `validate_vs_ensembldb.py` | Runs fastCDS + ensembldb, classifies into 6 buckets, emits Table 1 |
| `scaling_benchmark.py` | fastCDS vs ensembldb at N = 100 … 1 M |
