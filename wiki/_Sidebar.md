**[Home](Home)**

**1 · [How to install](Installation)**
- [pip](Installation#pip)
- [bioconda](Installation#bioconda)
- [pixi](Installation#pixi)
- [From source](Installation#build-from-source)

**2 · [Building an index](Index)**
- [Obtain a GTF](Index#1-obtain-a-gtf)
- [Index a local GTF](Index#2-index-a-local-gtf)
- [Fetch from Zenodo](Index#3-retrieve-a-pre-built-index-from-zenodo)

**3 · [Mapping](Mapping)**
- [Query BED](Mapping#input-the-query-bed)
- [Mapping a domain](Mapping#mapping-a-domain)
- [Transcript structure](Mapping#get-the-transcript-structure---output-isoform)
- [Output description](Mapping#output-description)

**4 · [Plotting](Plotting)**
- [Static figures](Plotting#static-figures)
- [Interactive viewers](Plotting#interactive-viewers)

**5 · [Tutorials and notebooks](Tutorials-and-Notebooks)**
- [Quickstart](Tutorials-and-Notebooks#quickstart-a-real-run-you-can-copy-paste)
- [Walkthrough](Tutorials-and-Notebooks#end-to-end-walkthrough-notebook)
- [Pfam atlas](Tutorials-and-Notebooks#pfam-proteome-atlas)
- [ClinVar](Tutorials-and-Notebooks#clinvar-enrichment)

**6 · [Performance and benchmarking](Performance-and-Benchmarking)**
- [Speed & memory](Performance-and-Benchmarking#speed-and-memory-usage)
- [Accuracy vs tools](Performance-and-Benchmarking#accuracy-vs-other-tools)
- [Speed vs tools](Performance-and-Benchmarking#speed-vs-other-tools)

**7 · Reference**
- [Python API](Python-API)
- [Architecture](Architecture)
- [FAQ](FAQ)
