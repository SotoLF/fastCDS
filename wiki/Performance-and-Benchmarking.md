...
This incomplete-CDS divergence is a **convention difference, not an error.** `cds_start_NF` means Ensembl could not locate the start codon (the true ATG is upstream of the assembled CDS), so the first codon is incomplete and "where residue 1 begins" is genuinely ambiguous. **prot2exon, ensembldb, and TransVar all anchor residue 1 at the first annotated CDS base** (effectively phase 0) and therefore agree exactly, including on every incomplete-CDS query. **Ensembl REST honours the GTF CDS `phase`**: it emits the missing base(s) as a `0-0` gap and shifts the mapping by the phase (1–2 nt), which propagates along the whole protein (and in one minus-strand case spills a single base into a distal exon → the lone `structural_mismatch`). prot2exon deliberately keeps the CDS-start anchor — the 2-of-3 majority convention — because adopting REST's phase convention would *break* agreement with ensembldb and TransVar; it touches only the ~4% of transcripts flagged incomplete.
