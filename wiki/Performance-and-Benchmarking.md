...
- **Ensembl REST (Ensembl 115):** **4,842 / 5,000 = 96.84%.** Every one of the 158 divergences (141 `off_by_one` + 17 `structural_mismatch`) falls on a `cds_start_NF` / `cds_end_NF` transcript — the `cds_incomplete` stratum drops to 76% while every other stratum stays at 95–99%, and on complete-CDS transcripts REST is also 100%.
