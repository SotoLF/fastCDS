...
def test_mapper_map_single(binary, with_tags_index, tmp_path):
    ...
    mapper = prot2exon.Mapper(index=str(with_tags_index), binary=str(binary))
...
def test_keep_outputs_then_read_results_dir(binary, with_tags_index, tmp_path):
    ...
    re_read = prot2exon.read_results_dir(str(persist))
    ...
    assert re_read.metadata.get("tool") == "prot2exon"


def test_plot_smoke(binary, with_tags_index, tmp_path):
    """`p2e.plot(MappingResult, out=...)` and `p2e.plot(DataFrame, ...)` both
    ...
    prot2exon.plot(result, out=str(pdf_a))
    assert pdf_a.exists() and pdf_a.stat().st_size > 0

    pdf_b = tmp_path / "from_df.pdf"
    prot2exon.plot(result.isoform, input_id="PLOT_ME", out=str(pdf_b))
